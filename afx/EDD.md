...
**Objective (ALGORITHM.md §5.1–5.3).** Each racer has a `prefer`: `finish` (default; en route is the other kind), `neutral` (en route is preferred, the finish the other kind), or `en_route` (en route preferred; the finish is worth only a repeat). Five levels in strict priority: (1) every racer had their preferred sighting, (2) every racer's finish seen, (3) each racer's preferred sighting, (4) each racer's other sighting, (5) repeat en-route sightings. Levels 1 and 2 are completeness bonuses earned by the sighting that completes the set, over racers with positive priority; 3 and 4 pay per racer, scaled by priority; level 5 follows a concave curve — the *k*-th repeat is worth `priority × repeat_decay^k`, with a fixed decay of one half, so a repeat is a bonus and never rivals a first sighting of somebody else. Each level's weight is a power of a *base* of ten times the most a level could accumulate for this field (`racers × max_priority × Σ decay^k`), which keeps the scalar lexicographic and within f64 precision for fields of hundreds. A racer's `require_finish` charges `base⁵` when their finish is missed, alongside level 2's bonus; a required region missed costs `base⁶`. Both are soft, so an impossible requirement still yields a plan, and both are part of the rank the beam and the final choice use. The beam key adds credit proportional to progress toward each completeness bonus so near-complete labels survive trimming.
