# birdeye — Prior Art, Stack Validation, and Risk Research
...
1. **There is no prior art close enough to matter.** Live race-tracking apps (RaceJoy,
   ...
   *validates* a user-chosen set of 1–5 stops against hardcoded travel-speed formulas; it
   ...
   supports six named marathons. birdeye is not duplicating anything.
2. **The academic framing is solid and the exact application is novel.** birdeye is an
...
carrying a phone). birdeye requires neither — it works from a course and a pace estimate,
...
This is precisely the plan birdeye would compute, and today it is produced entirely by hand.
...
they solve birdeye's *prediction* half well, and none of its *routing/optimization* half.
...
course or pace model) but it is direct evidence that **the exact algorithmic bet birdeye is
making runs fine in a browser**.

No purpose-built Rust or WASM orienteering solver was found. birdeye's solver would be
written from the published algorithms, not ported.

### 1.3 Academic framing

**birdeye's problem is MC-TOP-MTW.** Candidate viewing spots are nodes with a score. Each
...
both borrow the words as naming metaphors. **birdeye's application domain appears genuinely
...
| **Pulse framework (exact)** | Duque, Lozano, Medaglia (2014), "Solving the OPTW via the pulse framework", *C&OR* 54, 168–176. DOI: [10.1016/j.cor.2014.08.019](https://doi.org/10.1016/j.cor.2014.08.019) | Reported up to 266× speedup over the prior state of the art and optimal solutions on instances up to **562 nodes** — right at birdeye's upper size bound. The strongest candidate if you want a provably-optimal mode. |
...
**Use these as birdeye's optimizer test fixtures** — they give a hard, external correctness
...
- **AGPL §13 probably does not bite for a purely static app**, but the reasoning is worth
  ...
  users interact with remotely. birdeye's engine runs on the user's own machine; serving the
...
- **Patents:** a search over race-spectator patents turned up the Huston/Coleman
  ...
  (AR/VR-flavoured), not itinerary optimization. Nothing found that reads on birdeye. This is
...
wasm-bindgen --target web target/wasm32-unknown-unknown/release/birdeye.wasm --out-dir web/wasm
wasm-opt -Oz -o web/wasm/birdeye_bg.wasm web/wasm/birdeye_bg.wasm
```
...
For a project of birdeye's shape — geometry, a graph, a heuristic solver, no async runtime —
...
an engineering estimate, **[unverified]** for birdeye specifically). `serde_json` is
...
1. **GitHub Pages cannot set custom headers.** A GitHub staff reply in
   ...
   2,000 chars per line, applies to static assets — which is all birdeye has) and Netlify
...
"faster than JSON" claim does not hold for birdeye's data shape.** wasm-bindgen's own guide
...
birdeye doesn't need it: the course is its own polyline used as-is, and snapping candidate
...
`Marker` objects. birdeye's feature counts (a course, a few hundred candidate spots, an
...
([PR #854](https://github.com/tyrasd/overpass-turbo/pull/854)) — birdeye should treat the
...
way coordinates. For birdeye you actually need tags (to classify footway vs. cycleway vs.
...
them returning 200s. If twenty spectators at the same race open birdeye within a minute, they
...
**R2 — Modelling credibility (underrated).** birdeye's output is only as good as its arrival
...
6. **Make pace uncertainty a first-class UI concept, and add "I just saw them at X" re-solve.**
   This is the feature that turns R2 from a weakness into birdeye's differentiator, and
